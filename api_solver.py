...
class TurnstileAPIServer:
    ...
    def _setup_routes(self) -> None:
        ...
        self.app.route('/restart', methods=['POST'])(self.restart_server)
    ...
    async def restart_server(self):
        """Handle the /restart endpoint to restart the API server."""
        if self.debug:
            logger.info("Restart request received. Initiating server restart...")

        # Close all browser instances
        while not self.browser_pool.empty():
            try:
                _, browser = await self.browser_pool.get_nowait()
                await browser.close()
            except:
                pass
        ...
        async def delayed_restart():
            ...
            subprocess.Popen([python_executable, current_script] + current_args)
